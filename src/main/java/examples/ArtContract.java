...
public class ArtContract implements Contract {
    ...
    public void verify(LedgerTransaction tx) throws IllegalArgumentException {
        ...
        if (command.getValue() instanceof Commands.Issue) {
            if (tx.getInputStates().size() > 0) throw new IllegalArgumentException("Art create should have no inputs.");
            if (tx.getOutputStates().size() != 1) throw new IllegalArgumentException("Art create should have one output.");
            if (tx.outputsOfType(ArtState.class).size() != 1) throw new IllegalArgumentException("Art create output should be an ArtState.");
            ...
            if (!(requiredSigners.contains(artStateOutput.getOwner().getOwningKey())))
                throw new IllegalArgumentException("Art create should have output's owner as a required signer.");
